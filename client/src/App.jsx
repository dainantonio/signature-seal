...
const Navbar = ({ onBookClick, onViewChange, currentView }) => {
  ...
  return (
    <nav className={`fixed w-full top-0 z-40 transition-all duration-500 border-b ${scrolled ? 'bg-white/95 backdrop-blur-xl border-gray-100 py-2' : 'bg-transparent border-transparent py-5'}`}>
...
const AIChatWidget = ({ onRecommend }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', text: "I'm having trouble connecting. Please check your connection." }]);
    } finally {
      setIsLoading(false);
    }
  };
...
const BookingModal = ({ isOpen, onClose, initialService }) => {
  ...
  const submitBooking = async () => {
    ...
    } catch (err) { alert("Network Error: Could not connect to server."); } finally { setIsSubmitting(false); }
  };
        ...
        {success ? (
          ...
          <>
            <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-50">
              ...
              {/* EXIT BUTTON - High Z-Index, Large Touch Target */}
            ...
            <div className="p-8 overflow-y-auto bg-gray-50/30">
                ...
                <div className="space-y-8">
                  <div className="grid md:grid-cols-2 gap-4">
                    {['Mobile Notary', 'Loan Signing', 'Estate Planning', 'Vehicle Title', 'Remote Online Notary (OH Only)'].map(svc => (
                      <button key={svc} onClick={() => setFormData({...formData, service: svc})} className={`p-5 rounded-2xl text-left border ${formData.service === svc ? 'bg-brand-navy-dark text-white border-brand-navy-dark' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <input type="date" className="w-full p-3 border border-gray-200 rounded-xl" onChange={(e) => setFormData({...formData, date: e.target.value})} value={formData.date}/>
                    ...
                    <div className="space-y-2">
                        ...
                        <select className="w-full p-3 border border-gray-200 rounded-xl" onChange={(e) => setFormData({...formData, time: e.target.value})} value={formData.time} disabled={!formData.date || timeSlots.length === 0}>
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <input type="text" placeholder="Full Name" className="w-full p-4 border border-gray-200 rounded-xl" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                  <input type="email" placeholder="Email Address" className="w-full p-4 border border-gray-200 rounded-xl" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                  <textarea placeholder="Meeting Address & Instructions" rows={3} className="w-full p-4 border border-gray-200 rounded-xl" value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} />
                  <textarea placeholder="Additional Notes (Optional)" rows={2} className="w-full p-4 border border-gray-200 rounded-xl" value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} />
                </div>
              )}
              {step === 3 && (
...
const AdminDashboard = ({ token, onLogout }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // 2. If DELETE fails, try POST Fallback
      if (!res.ok) {
         console.warn("DELETE method blocked, trying fallback POST...");
      ...
      if (!res.ok) {
        if (res.status === 401 || res.status === 403) {
            ...
            alert("Delete failed on server. Please refresh.");
        }
      }
    } catch (err) {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bookings.map(booking => (
            <div key={booking.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative">
              ...
              {/* DELETE BUTTON - MOVED TO FOOTER FOR BETTER ACCESS */}
...
// Page Sections (Hero, Services, WhyUs, Pricing, Footer) omitted for brevity but remain unchanged from previous stable version
