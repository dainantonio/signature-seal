      ...
      <div className="md:hidden container mx-auto px-6 h-24 grid grid-cols-[1fr_auto_1fr] items-center">
        ...
        <div className="justify-self-center flex flex-row items-center gap-3 cursor-pointer w-full justify-center" onClick={() => onViewChange('home')}>
          ...
          <div className="flex flex-col justify-center items-center">
            ...
            <span className={`text-[10px] sm:text-xs leading-none uppercase font-bold mt-1 whitespace-nowrap text-center tracking-widest ${scrolled ? 'text-brand-teal' : 'text-gray-300'}`}>WV Notary</span>
          </div>
        </div>
            ...
            <div className="p-8 overflow-y-auto bg-gray-50/30">
              ...
              {step === 2 && (
                <div className="space-y-5">
                  ...
                  <textarea placeholder="Address (Huntington, WV area)" rows={3} className="w-full p-4 border rounded-xl" value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} />
                ...
                <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-lg space-y-6">
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    <label className="flex items-center gap-3 cursor-pointer p-4 border rounded-xl hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1"><span className="font-bold text-brand-navy-dark flex items-center gap-2"><CreditCard size={18}/> Pay Deposit Online</span><p className="text-xs text-gray-500">Secure pay via Stripe</p></div>
                    </label>
                  </div>
                </div>
              )}
    ...
    <div className="min-h-screen bg-brand-navy-dark flex items-center justify-center p-6">
      <div className="bg-white p-10 rounded-3xl text-center shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-brand-navy-dark text-white font-bold py-4 rounded-xl">Unlock</button>
        </form>
      </div>
    </div>
  );
  ...
  const fetchBookings = (page = 1) => {
    ...
    safeFetch(`${API_URL}/api/bookings?page=${page}&limit=9`, { headers: { 'Authorization': `Bearer ${token}` } })
      ...
      .then(data => {
        ...
        else if (data.data) { setBookings(data.data); setPagination(data.pagination); }
        setLoading(false);
      })
    ...
    <div className="container mx-auto px-6 py-24 min-h-screen bg-gray-50 pt-32">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="flex gap-4">
          <button onClick={handleExport} className="flex items-center gap-2 bg-brand-navy-dark text-white px-4 py-2 rounded-lg text-sm"><Download size={16}/> Export</button>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bookings.map(b => (
            <div key={b.id} className="bg-white p-6 rounded-2xl shadow-sm border relative">
              <button onClick={() => handleDelete(b.id)} className="absolute top-6 right-6 text-gray-300 hover:text-red-500"><Trash2 size={18}/></button>
              <h3 className="font-bold text-lg">{b.name}</h3>
              <p className="text-sm text-gray-500">{b.service}</p>
              <div className="text-sm text-gray-600 mt-4"><p>📅 {new Date(b.date).toLocaleDateString()} at {b.time}</p><p>📍 {b.address}</p></div>
            </div>
          ))}
        </div>
      )}
  ...
  <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
    ...
    <div className="container mx-auto px-6 relative z-10 pt-40 md:pt-20 text-center">
      <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="max-w-4xl mx-auto">
        <div className="inline-block px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md text-brand-gold text-xs font-bold uppercase tracking-widest mb-10 border border-white/10">Serving Huntington, WV & South Point, OH</div>
        ...
        <p className="text-lg md:text-2xl text-gray-300 mb-12 max-w-2xl mx-auto font-light">Certified mobile notary services delivered to your doorstep. Accuracy you can rely on, on your schedule.</p>
...
const Pricing = ({ onBookClick }) => (
  <section id="pricing" className="py-32 bg-gray-50">
    <div className="container mx-auto px-6">
      <div className="text-center mb-20"><h2 className="text-4xl md:text-5xl font-serif font-bold text-brand-navy-dark mb-4 tracking-tight">Pricing</h2><p className="text-xl text-gray-500">Transparent & Compliant</p></div>
      <div className="max-w-md mx-auto">
        <div className="bg-white p-12 rounded-[3rem] shadow-xl border border-gray-100 flex flex-col items-center group hover:shadow-2xl transition-all">
          ...
          <div className="flex items-baseline mb-10 text-brand-navy-dark">
            <span className="text-5xl font-serif font-bold">From $40</span>
          </div>
          <ul className="space-y-4 mb-12 text-gray-600 w-full text-sm">
            {[
              '$10 per notarized signature (WV State Law)',
              'Travel fee starts at $40 (covers 10 miles)',
              'Mileage surcharge for 10+ miles',
              'Fees disclosed and agreed upon before service'
            ].map(item => (
