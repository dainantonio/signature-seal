...
const iconVariants = {
  drive: { hover: { x: [0, -3, 3, -3, 3, 0], transition: { duration: 0.5 } } }, // Car shimmy
  nod: { hover: { rotate: [0, -10, 10, -10, 0], transition: { duration: 0.5 } } }, // Document check
  pulse: { hover: { scale: [1, 1.1, 1], transition: { duration: 0.8, repeat: Infinity } } }, // Shield heartbeat
  rush: { hover: { x: [0, 10, -5, 0], transition: { duration: 0.4 } } } // Truck zoom
};
      ...
      <div className="hidden md:flex container mx-auto px-6 justify-between items-center h-24"> 
        <div className="flex items-center gap-4 cursor-pointer group select-none" onClick={handleRefresh} title="Refresh Page">
          ...
          <div className="flex flex-col justify-center items-center"> 
            ...
            <span className={`text-xs leading-none tracking-[0.2em] uppercase font-bold mt-1.5 text-center ${scrolled ? 'text-brand-teal' : 'text-gray-300'}`}>WV Mobile Services</span>
          </div>
        </div>
      ...
      <div className="md:hidden container mx-auto px-6 h-24 grid grid-cols-[1fr_auto_1fr] items-center">
        ...
        <div className="flex flex-row items-center gap-3 cursor-pointer justify-center" onClick={handleRefresh}>
          ...
          <div className="flex flex-col justify-center items-center">
            ...
            <span className={`text-[10px] uppercase font-bold mt-1 tracking-widest ${scrolled ? 'text-brand-teal' : 'text-gray-300'}`}>WV Mobile Services</span>
          </div>
        </div>
...
const QRModal = ({ isOpen, onClose }) => {
    ...
    const downloadQR = async () => {
        ...
        } catch (err) { alert("Could not download image. Please screenshot."); }
    };
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-brand-navy-dark/80 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl relative" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={downloadQR} className="w-full flex items-center justify-center gap-2 bg-brand-navy-dark text-white py-3 rounded-xl font-bold hover:bg-brand-teal transition-all"><Download size={18} /> Save Image</button>
            </div>
        </div>
    );
};
...
const FAQ = () => {
  ...
  const faqs = [
    ...
    { q: "What is 'Legal Document Courier'?", a: "We provide secure, white-glove pickup and delivery for court filings, real estate closing packages, and sensitive legal documents in the Tri-State area." },
    ...
    { q: "How does pricing work?", a: "We charge a standard Travel Fee (starting at $40) to come to you. The state-regulated Notary Fee ($10 per stamp in WV) is separate and collected at the appointment. Courier and I-9 services are flat rates." },
  ];
  return (
...
const AIChatWidget = ({ onRecommend }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([{ role: 'assistant', text: "Hi! I'm the Concierge. I can help with I-9 Verification, Mobile Notary, Courier Services, and scheduling. How can I assist?" }]);
...
const BookingModal = ({ isOpen, onClose, initialService }) => {
  ...
  const isCourier = formData.service.includes('Courier');
  const isFlatFee = isI9 || isCourier;

  const price = useMemo(() => {
    let base = 40;
    if (formData.service.includes('Loan')) base = 150;
    if (isI9) base = 60; // $60 Flat
    if (isCourier) base = 55; // $55 Flat
  ...
  }, [formData.service, formData.mileage, formData.signatures, formData.locationType, isFlatFee, isI9, isCourier]);
  ...
  const timeSlots = useMemo(() => {
    ...
    if (day === 0) return []; 
    // Flexible Hours for I-9 & Courier
    if (isFlatFee) {
        const slots = [];
        for (let i = 9; i <= 19; i++) {
              ...
              {step === 1 && (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    {/* NEW SERVICE LIST */}
                    {['Mobile Notary (WV)', 'I-9 Verification (Authorized Rep)', 'Legal Document Courier', 'Oaths & Affirmations'].map(svc => (
                    ...
                    <label className="flex items-start gap-3 p-4 border border-brand-gold/30 bg-orange-50/50 rounded-xl cursor-pointer">
                        ...
                        <span className="text-xs text-gray-700 leading-relaxed">
                            {isI9 
                                ...
                                : isCourier
                                    ? "I understand this is a Secure Courier service for document transport. The fee covers pickup, travel, and delivery."
  ...
  <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
    ...
    <div className="container mx-auto px-6 relative z-10 pt-40 md:pt-20 text-center">
      <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="max-w-4xl mx-auto">
        ...
        <p className="text-lg md:text-2xl text-gray-300 mb-12 max-w-2xl mx-auto font-light">Certified mobile notary, I-9 verification, and legal courier services delivered to your doorstep in West Virginia. Accurate, professional, and ready.</p>
    ...
    <div className="container mx-auto px-6">
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-3 gap-10">
        {[
          ...
          { icon: Truck, title: "Secure Courier", desc: "White-glove delivery for sensitive legal & real estate documents.", variants: iconVariants.rush }
        ].map((s, i) => (
