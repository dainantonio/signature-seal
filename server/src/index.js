...
// --- PERMISSIVE CORS (Fixes Network Errors) ---
app.use(cors({ origin: '*' })); 
app.options('*', cors());
...
app.post('/api/create-checkout-session', async (req, res) => {
  if (!stripe) {
    return res.status(500).json({ error: "Stripe is not configured on the server. Check Render environment variables." });
  }
  ...
  else if (service.includes('Remote')) amount = 5000;
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'usd',
          product_data: { name: service, description: `${date} at ${time}` },
          unit_amount: amount,
        },
        quantity: 1,
      }],
